...
(** 以下の表明を日本語に直しなさい。
[[
      fun st =>  asnat (st X) = 3     ---> X = 3 であることを表明する。

      fun st =>  asnat (st X) = x     ---> assert that X = x

      fun st =>  asnat (st X) <= asnat (st Y)       ---> assert that X <= Y

      fun st =>  asnat (st X) = 3 \/ asnat (st X) <= asnat (st Y)      ---> X = 3 or X <= Y

      fun st =>  (asnat (st Z)) * (asnat (st Z)) <= x 
                 /\ ~ (((S (asnat (st Z))) * (S (asnat (st Z)))) <= x)  
           --> Z^2 <= x かつ x < (Z+1)^2  つまりxの平方根の整数部分を求める?

      fun st =>  True      ---> assert True

      fun st =>  False     --- >assert False
]]
[] *)
...
[[
      {{True}} c {{X = 5}}
    もし [c] が表明 True を満たす状態から開始され、また、
    [c]がいつかは停止するならば、最終状態では、表明 X = 5 が成立することを保証する。

      {{X = x}} c {{X = x + 5)}}
    もし [c] が表明 X = x を満たす状態から開始され、また、
    [c]がいつかは停止するならば、最終状態では、表明 X = x + 5が成立することを保証する。

      {{X <= Y}} c {{Y <= X}}
    もし [c] が表明 X <= Y を満たす状態から開始され、また、
    [c]がいつかは停止するならば、最終状態では、表明 Y <= X が成立することを保証する

      {{True}} c {{False}}
    もし [c] が表明 True を満たす状態から開始され、また、
    [c]がいつかは停止するならば、最終状態では、表明 False が成立することを保証する。

      {{X = x}}
      c
      {{Y = real_fact x}}.
    もし [c] が表明 X = x を満たす状態から開始され、また、
    [c]がいつかは停止するならば、最終状態では、表明 Y = real_fact x が成立することを保証する。

      {{True}}
      c
      {{(Z * Z) <= x /\ ~ (((S Z) * (S Z)) <= x)}}
    もし [c] が表明 True を満たす状態から開始され、また、
    [c]がいつかは停止するならば、最終状態では、
    表明 x は Zの2乗以上でかつ x は (Z+1)の2乗未満 であることが成立することを保証する。
]]
 *)
(** [] *)
...
[[
      ...
      {{True}} X ::= 5; Y ::= 0 {{X = 5}}   : valid 

      {{X = 2 /\ X = 3}} X ::= 5 {{X = 0}}    :   valid

      {{True}} SKIP {{False}}    : invalid 

      {{False}} SKIP {{True}}    : invalid -> valid? 

      {{True}} WHILE True DO SKIP END {{False}}    : valid
...
[[
     ...
     IFB X == 0 THEN Y ::= Z + 1 ELSE Y ::= W + 2 FI
     {{ Y = 5 }}
        ===>  X <> 0, W = 3 ?        ( X = 0, Z = 4 is harder )  --> (X =0 /\ Z =4 ) \/ (X <>0 /\ W =3)
     ...
     WHILE True DO X ::= 0 END
     {{ X = 0 }}
        ===> False, X = 0 ?  gaitou nashi?    --> True   (False は最も厳しい条件. Trueは最も緩い)
]]
*)
(** [] *)
...
(** ホーア論理のゴールは、ホーアの三つ組の正しさを証明する"合成的"手法を提供することです。
    ...
    このゴールのために、以降の節では、Impのコマンドのいろいろな構文要素のそれぞれに対して、
...
[[
    ...
    この規則の問題点を指摘しなさい。
       ...
       ---> 帰結のところから.  [True] と [3 = 3] は、構文的に等しい表明ではない ??   ---> wrong?
...
Proof.
  intros P V a Q H st H0. unfold assn_sub.
  apply H with (st := st).
  apply E_Asgn.
  reflexivity.
  assumption.
...
Proof.
  ...
  apply hoare_consequence with (P' := P') (Q' := Q). try assumption. try assumption.
...
Proof.
  eapply hoare_seq.
    eapply hoare_asgn.
    apply hoare_consequence_post with (fun st => asnat (st X) = 1) .
    ...
    intros st H. unfold assn_sub, update. simpl. split. assumption. reflexivity. 
Qed.
...
*)
Definition exchange_val : com :=
  (Z ::= AId X;
   X ::= AId Y;
   Y ::= AId Z).

Example exchange_val_prove :
  {{fun st => asnat (st X) <= asnat (st Y) }} exchange_val {{fun st => asnat (st Y) <= asnat (st X) }}.
Proof.
  unfold exchange_val. eapply hoare_seq. eapply hoare_seq.
  apply hoare_asgn.
  apply hoare_asgn.
  eapply hoare_consequence. (* with (fun st => asnat (st Z ) = asnat (st X )) (fun st => asnat (st Y) = asnat (st Z )).*)
    apply hoare_asgn.
    intros st H. unfold assn_sub. unfold aeval, update.
    remember (st Z) as Q. destruct Q.
...
*)
(*
Example  hoare_asgn_example5 : forall a n, (* forall (a : aexp) (n : nat), *)
         {{fun st => aeval st a = n}} (X ::= (ANum 3); Y ::= a)
         {{fun st => asnat (st Y) = n}}.
Proof.
...
Proof.
  ...
  inversion HE; subst.
  Case "b is true".  (* eapply HTrue. eassumption. *)
    apply (HTrue st st').  
...
Lemma hoare_repeat_firsttry : forall P b c,   (* wrong *)
  {{fun st => P st}} c {{P}} ->
...
Proof.
  ...
  Case "E_RepeatEnd".
    split. apply Hhoare in He. apply He. assumption.
    apply bexp_eval_false. assumption.

  Case "E_RepeatLoop".
    apply IHHe2.  reflexivity.
    eapply Hhoare; try eassumption.
    (* apply (Hhoare st st'); try assumption. *)
Qed.

Lemma hoare_repeat : forall P b c,   (* Gotoh-san ver. *)
  {{fun st => P st}} c {{P}} ->
  {{P}} REPEAT c UNTIL b END {{fun st => P st  /\ (bassn b st) }}.
Proof.
...
*)
(** ホーア論理の一番のポイントは、合成的ということです。
   証明の構造は常にプログラムの構造に従います。
...
*)
 ...
 (* ここから担当。
    https://github.com/nagaet/sfja/blob/master/Hoare_J.v  にソースがあります。 *)
...
(*
 ここで https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare.v  
  の方法を参考にした方法をご紹介します。 (#proofcafe の中の人らしいです。)
*)

(* 非形式的証明において、'=>' に以下のように番号(1), (2), (3) をつけます。
      {{ X = x /\ Z = z }} =>
      {{ Z - X = z - x }}           ... (1)
    WHILE X <> 0 DO
        {{ Z - X = z - x /\ X <> 0 }} =>
...
Proof.
  intros x z. unfold subtract_slowly.
  eapply hoare_consequence_post.      (* (3) *)

  eapply hoare_consequence_pre.    (* (1) *)

  apply hoare_while with (P := fun st => asnat (st Z) - asnat (st X)  = z - x).    (* (2) *)
  eapply hoare_consequence_pre.
  ...
  remember (beq_nat (asnat (st X)) 0) as Q; destruct Q.
    inversion GuardTrue. 
    symmetry in HeqQ. apply beq_nat_false in HeqQ. omega.
...
Proof.
  ...
  intros st  [H0 H1]. apply H0.

  unfold  assn_sub, update, bassn.  
  intros st [H0 H1]. simpl in H1. unfold beval, not in H1. unfold negb in H1.
...
(*上記と同様に
  https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare.v の方法で証明してみます.
*)

(* 非形式的証明において、'=>' に以下のように番号をつけます。
[[
          {{ True }}
        WHILE X <> 0 DO
            {{ True /\ X <> 0 }} =>
...
Definition add_slowly_invariant x z :=
  fun st => plus (asnat (st Z)) (asnat (st X)) = plus z x.
...
Proof.
  ...
  Case "loop body preserves invariant".
    ...
    remember (beq_nat (asnat (st X))0) as  Q; destruct Q. 
    inversion GuardTrue. symmetry in HeqQ. apply beq_nat_false in HeqQ.
    omega.

  Case "initial state satisfies invariant".
    intros st [HX HZ]. unfold add_slowly_invariant. omega.

  Case "invariant and negated guard imply postcondition ".
    intros st [Inv GuardFalse]. unfold add_slowly_invariant in Inv.
    unfold bassn in GuardFalse. simpl in GuardFalse.
    destruct (asnat (st X)). omega.
    apply ex_falso_quodlibet. apply GuardFalse. reflexivity.
Qed.
*)
...
(* 同様に
  https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare.v の方法で証明します.
*)
...
[[
  ...
  Y ::= 0;
    {{ X = x /\ Y = 0 }} =>
    {{ (Y=0 <-> ev (x-X)) /\ X<=x }}     (* ev_0: ev 0 | ev_SS : forall n : nat, ev n -> ev (S (S n)) *)    (* <--??? *)
  WHILE X <> 0 DO
      {{ (Y=0 <-> ev (x-X)) /\ X<=x /\ X<>0 }} =>
...
Definition find_parity : com :=      (* Xが偶数ならばY=0。そうでなければY=1 *)
  Y ::= (ANum 0);
  WHILE (BNot (BEq (AId X) (ANum 0))) DO
...
(*  https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare.v の方法
[[
    {{ X = x }} =>
    {{ X = x /\ 0 = 0 }}        ... (1) 
  Y ::= 0;
...
Proof.
  ...
  simpl in GuardTrue. destruct (asnat (st X)).
    ...
    split.
      ...
      inversion Inv2 as [[H1 H2] | [H1 H2]]; rewrite H1;
        ...
        apply not_ev_ev_S in H2.
        replace (S (x - S n)) with (x - n) in H2. (*by omega.*)
        rewrite <- minus_n_O. assumption. omega.
  ...
  (* (1) の修飾の証明
     {{ X = x }} => {{ X = x /\ 0 = 0 }} *)
  intros st H.
  unfold assn_sub, find_parity_invariant, update. simpl. subst.
  split.
    omega.
    replace (asnat (st X) - asnat (st X)) with 0 by omega.
    left.
...
(* Definition find_parity_invariant x :=
  fun st =>
    asnat (st X) <= x   /\  (asnat (st Y) = 0 /\ ev (x - asnat (st X))  \/  asnat (st Y) = 1 /\   ~ev (x - asnat (st X))). *)
...
(* ---> ループが奇数回回るときの 不変条件  asnat (st Y) = 1 /\ ~ev (x - asnat (st X))) がない?  *)
(** [] *)
...
Definition sqrt_spec (x : nat) : Assertion :=         (* Z^2 <= x <(Z+1)^2 *)
  fun st =>
...
(** 上記の正しさの証明に対応する修飾付きプログラムを記述しなさい。*)
  ...
  (* 参考:  Theorem sqrt_com_correct : forall x,
      {{fun st => True}} (X ::= ANum x; sqrt_com) {{sqrt_spec x}}. *)
...
(* 以下は
  https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare.v の方法です 
*)

Theorem sqrt_com_correct' : forall x,
  {{fun st => True}} (X ::= ANum x; sqrt_com) {{sqrt_spec x}}.
Proof.
  intros x. unfold sqrt_com.
  ...
  inversion H as [[HX HZ] HP]. split.
    rewrite update_neq; auto.
    rewrite (update_eq (aeval st (APlus (ANum 1) (AId Z))) Z st). subst. apply random_fact_2. assumption.
  ...
  split.
    ...
    intros contra. apply HP. clear HP.
    simpl. simpl in contra.
    apply random_fact_1. subst x. apply contra. 
  ...
  eapply hoare_consequence_pre.        (* (0)  *)
...
[[
    {{ X = x }} =>
    {{ real_fact X  = real_fact x  }}     <-----???????
  Z ::= X;
    {{ real_fact Z = real_fact x }} =>
    {{ 1 * real_fact Z  = real_fact x  }}
  Y ::= 1;
...
Theorem fact_com_correct_firsttry : forall x,
  {{fun st => asnat (st X) = x}} fact_com
  {{fun st => asnat (st Y) = real_fact x}}.
Proof. 
  intros x. unfold fact_com.
  apply hoare_consequence_pre with (P' := fun st => real_fact (asnat (st X)) = (real_fact x)).

  apply hoare_seq with (Q := fun st => asnat (st Z) = asnat (st X)).

  Case "fact_loop".
    unfold fact_loop. 
    eapply hoare_seq with (Q := fun st => asnat (st Y) = 1).
    ...
    SCase "fact_body".
      ...
      SSCase "loop preserves invariant".
        ...
        intros st [HI HZ]. unfold assn_sub. unfold fact_inv. simpl.
        rewrite -> update_neq; try reflexivity. unfold fact_inv in HI.
 
        assert  (asnat (st Z) * real_fact (asnat (st Z) -1) = real_fact (asnat (st Z))) as H0.
          admit.  (* ???? *)

        rewrite -> mult_assoc_reverse. rewrite -> H0. assumption.

        intros st H. unfold fact_inv. unfold real_fact. rewrite -> H. simpl. rewrite -> plus_0_r. destruct (st Z). 
        admit. admit.

        intros st [H0 H1]. unfold fact_inv in H0. rewrite <- H0. unfold real_fact.
        unfold not, bassn in H1. apply ex_falso_quodlibet. apply H1. unfold beval. 
        admit.

        admit. (*
        eapply hoare_consequence. apply hoare_asgn.
        intros st H. unfold assn_sub, assn_sub, aeval.    *)
      ...
      (*
      SSCase "after loop, spec is satisfied".
        intros st H. inversion H as [HI HZ]. unfold not in HZ. unfold bassn in HZ.
        unfold fact_inv in HI. admit.  (* <- ???*)
      *)
...
(* 
 from  https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare.v を参考に
[[
    {{True}} =>
    {{ X = x }}        ... (1)
  Z ::= X;
  ...
  Y ::= 1;
    {{Z = x /\ Y = 1}} =>
    {{Y*real_fact(Z) = real_fact(x)}}     ... (3),  ループ不変式 
  WHILE Z <> 0 DO
      {{Y * real_fact(Z) = real_fact(x) /\ Z <> 0}} =>       .... ループ不変式、かつ、ループ実行条件
      {{Y * Z * real_fact(Z - 1) = real_fact(x)}}          ... (4) 
    Y ::= Y * Z;
      {{Y * real_fact(Z - 1) = real_fact(x)}}
    Z ::= Z - 1
      {{Y * real_fact(Z) = real_fact(x)}} 
...
Theorem fact_com_correct : forall x,
  ...
  {{fun st => asnat (st Y) = real_fact x}}.
Proof. 
  unfold fact_com, fact_loop, fact_body. intros x.

  eapply hoare_consequence_post.            (* (5) *)         (* hoare_consequence_post を使うのは(5)のここだけ。*)
  ...
  apply hoare_while with (P := fact_inv x).
  eapply hoare_consequence_pre.             (* (4) *)
...
(*
   (4)の証明
     {{Y * real_fact(Z) = real_fact(x) /\ Z <> 0}}     (ループ不変式、かつ、ループ実行条件)
      => {{Y * Z * real_fact(Z - 1) = real_fact(x)}}
 *)
  unfold assn_sub, bassn, fact_inv, update. intros st [H1 H2]. simpl. 
  simpl in H2. remember (asnat (st Z)) as z. destruct z.
    simpl in H2. inversion H2.
    rewrite <- H1. rewrite <- mult_assoc. replace (S z - 1) with z by omega. reflexivity.
...
*)
  intros st [H1 H2]. unfold fact_inv. rewrite -> H1. rewrite -> H2. omega.
...
(*
   (1)の証明
     {{True}} => {{ X = x }}               (??? {{ X = x }}  => {{ X = x }}  ???)
*)
  intros st H. unfold assn_sub, assn_sub, update. simpl. assumption.
...
(*
   (5)の証明
     {{Y * real_fact(Z) = real_fact(x) /\ Z = 0}}  (ループ不変式、かつ、終了条件)
      => {{ Y = real_fact x }}  (要求仕様)
*)
  unfold fact_inv, update, bassn, assn_sub. intros st [H1 H2].
  remember (asnat (st Z)) as z. destruct z. 
    rewrite <- H1. simpl. omega.
    apply ex_falso_quodlibet. apply H2. simpl. unfold negb.  rewrite <- Heqz. simpl. reflexivity.
Qed.
...
Y := 0;
   {X = l /\ Y = 0} => 
   {Y + sum X = sum l}     ... (2)、ループ不変式
WHILE (isCons X) DO
   {Y + sum X = sum l /\ isCons X}  =>            (ループ不変式かつループ実行条件)
   {Y + head X + sum (tail X) = sum l}      ... (3)
  Y := Y + head X;
   {Y + sum (tail X) = sum l}
  X := tail X;
...
(*
上記の非形式的証明を参考にした形式的証明
*)
Theorem sum_program_correct_firsttry : forall l,
  {{ fun st => aslist (st X) = l }}
  sum_program
  {{ fun st => asnat (st Y) = sum l }}.
Proof.
  intros l.
  unfold sum_program.
  eapply hoare_consequence_post.  (* (4) *)         (* を使うのは(4)のここだけ。 *)

  eapply hoare_consequence_pre.   (* (1) *)

  apply hoare_seq with (Q := fun st =>  aslist (st X) = l /\ asnat (st Y) = 0).
  eapply hoare_consequence_pre.    (* (2) *)
  ...
  cut (head (aslist (st X)) + sum (tail (aslist (st X))) = sum (aslist (st X))). intros H.
  rewrite -> H. apply H1.
  induction (st X);  simpl.
    reflexivity.
    destruct l0; reflexivity.

  (*  (2)の証明 
    {X = l /\ Y = 0} => {Y + sum X = sum l}      (2)、ループ不変式 *)
  intros st [H1 H2]. rewrite -> H2. rewrite -> H1. reflexivity.

  apply hoare_asgn.    (* Y ::= ANum 0 *)

  (* (1)の証明 
    {X = l} => {X = l /\ 0 = 0}              (1)  *)
  intros st H. unfold assn_sub, update. simpl. rewrite -> H. split; reflexivity.
...
[[
  ...
  WHILE (BIsCons X)
  DO
      {{ Y = n /\ (exists p, p ++ X = l /\ (Z = 1 <-> appears_in n p))  /\ (BIsCons X) }}
    IFB (Y == head X) THEN
        {{ Y = n /\ (exists p, p ++ X = l        /\ (Z = 1 <-> appears_in n p)) /\ (BIsCons X) /\ Y == AHead X }} =>
        {{ Y = n /\ (exists p, p ++ tail X = l  /\ (1 = 1 <-> appears_in n p)) }}
      Z ::= 1
        {{ Y = n /\ (exists p, p ++ tail X = l  /\ (Z = 1 <-> appears_in n p)) }}
    ELSE
        {{ Y = n /\ (exists p, p ++ X = l /\ (Z = 1 <-> appears_in n p)) /\ (BIsCons X) /\ ~ (Y == head X) }} =>
        {{ Y = n /\ (exists p, p ++ tail X = l /\ (Z = 1 <-> appears_in n p)) }}
      SKIP
        {{ Y = n /\ (exists p, p ++ tail X = l /\ (Z = 1 <-> appears_in n p)) }}
    FI;
...
Fixpoint snoc {X:Type} (l:list X) (v:X) : (list X) :=      (* snoc l v で、l の末尾にv をつける。*)
  match l with
  | nil      =>  [ v ]
  | cons h t => h :: (snoc t v)      (* h: head,  t: tail *)
  end.
...
Proof.
  intros l n.
  eapply hoare_consequence. unfold list_member.
...
[[
  ...
  DO
      {{ Y = n /\ (exists p, p ++ X = l /\ (Z = 1 <-> appears_in n p))  /\ (BIsCons X) }}
    IFB (Y == head X) THEN
        {{ Y = n /\ (exists p, p ++ X = l        /\ (Z = 1 <-> appears_in n p)) /\ (BIsCons X) /\ Y == AHead X }} =>    (2)
        {{ Y = n /\ (exists p, p ++ tail X = l  /\ (1 = 1 <-> appears_in n p)) }}
      Z ::= 1
        {{ Y = n /\ (exists p, p ++ tail X = l  /\ (Z = 1 <-> appears_in n p)) }}
    ELSE
        {{ Y = n /\ (exists p, p ++ X = l /\ (Z = 1 <-> appears_in n p)) /\ (BIsCons X) /\ ~ (Y == head X) }} =>    (3)
        {{ Y = n /\ (exists p, p ++ tail X = l /\ (Z = 1 <-> appears_in n p)) }}
      SKIP
        {{ Y = n /\ (exists p, p ++ tail X = l /\ (Z = 1 <-> appears_in n p)) }}
    FI;
    X ::= ATail X
        {{ Y = n  /\ (exists p, p ++ X = l /\ (Z = 1 <-> appears_in n p)) }}
  END
     {{ Y = n /\ (exists p, p ++ X = l /\ (Z = 1 <-> appears_in n p)) /\ ~ (BIsCons X) }} =>     (4) 
     {{ Z = 1 <-> appears_in n l }}
]] *)
...
Proof.
  ...
  apply hoare_while with (P := fun st =>
     st Y = VNat n
     /\ exists p, p ++ aslist (st X) = l
                  /\ (st Z = VNat 1 <-> appears_in n p)).    (* loop invariant *)
  eapply hoare_seq.
  apply hoare_asgn.    (* X ::= ATail X *)
  apply hoare_if.

  Case "If taken".
    eapply hoare_consequence_pre.      (* (2) *)
    apply hoare_asgn.        (* Z ::= 1 *)
  ...
  unfold bassn in H5. unfold beval in H5. unfold aeval in H5.
  destruct (aslist (st X)) as [|h x'].
    rewrite append_nil in H2. rewrite <- H2. assumption.
    apply ex_falso_quodlibet. apply H5. reflexivity.
Qed.
...
[[
    {{ X = l /\ Y = nil }} =>     
    {{ rev X ++ Y = rev l  }}      (1)  (loop invariant)
  WHILE BIsCons (AId X) DO
      {{ rev X ++ Y = rev l  /\ (BIsCons X) }} => 
      {{ rev (tail X) ++ ((head X) :: Y) = rev l    }}     (2) 
    Y ::= ACons (AHead (AId X)) (AId Y);
      {{ rev (tail X) ++ Y  = rev l    }}
    X ::= ATail (AId X)
      {{ rev X ++ Y  = l    }}
...
Proof.
  ...
  apply hoare_while with (P := fun st => rev (aslist (st X)) ++ aslist (st Y) = rev l).        (* loop invariant *)
  eapply hoare_consequence_pre.      (* (2) *)

  eapply hoare_seq.
  apply hoare_asgn.      (* X ::= ATail (AId X) *)
  apply hoare_asgn.     (* Y ::= ACons (AHead (AId X)) (AId Y) *)
...
(** 既に定義されているコマンド[com]の記法[Notation]との衝突を避けるため、
    ...
    プログラムのトップで一番最初の事前条件を与える意図で用意したものです。*)
...
(** [dcom]の定義のどこに表明を置くかの選択は、ちょっと微妙です。
    ...
    しかしそうすると、同じアノテーションが繰替えされて、
    とてもうるさいプログラムになってしまうでしょう。
...
(** この関数は、最弱事前条件を計算する、というような洗練されたことは何もしません。
    ...
    [pre]と[post]を使い、
    修飾付きプログラムの一番最初には常に明示的な事前条件のアノテーションを
    付ける慣習を守ることを仮定すると、
...
(** ここまで見てきたタクティックだけを使うことで
    これらの命題の証明を進めることは確かにできるのですが、
...
(** [] *)
(*
Example slow_assignment_dec (x : nat) (y : nat) : dcom := (
    {{ fun st => True }}
    (*=>
    {{ fun st => assigning }}  *)
  X ::= (ANum x) 
  ...
  WHILE BNot (BEq (AId X) (ANum 0))
  DO   {{ fun st => asnat (st X) + asnat (st Y) = x
                 /\ bassn (BNot (BEq (AId X) (ANum 0))) st }}
     X ::= AMinus (AId X) (ANum 1)
...
(* first try *)
Example slow_assignment_dec_firsttry (x : nat) (y : nat) : dcom := (
    {{ fun st => True }}
  X ::= (ANum x) 
    {{ fun st => asnat (st X) = x }} ;
  Y ::= ANum 0
...
Theorem slow_assignment_dec_correct_firsttry : forall x y,
  dec_correct (slow_assignment_dec_firsttry x y ).
Proof.
  intros. verify.

  (* subgoal: beq_nat (asnat (st X)) 0 = true *)
  unfold not in H0. rewrite negb_true_iff in H0. rewrite -> beq_nat_false_iff in H0.
  rewrite -> beq_nat_true_iff. omega.

  (* subgoal:  beq_nat (asnat (st X)) 0 = true *)
  unfold not. intros H1. rewrite -> beq_nat_true_iff in H0. rewrite -> H0 in H1. simpl in H1.  inversion H1.

  (* subgoal:  asnat (update st X (VNat (asnat (st X) - 1)) Y) + (asnat (st X) - 1) + 1 = x *)
  rewrite -> update_neq. rewrite -> plus_comm. simpl. rewrite -> plus_n_Sm. 
  rewrite -> negb_true_iff in H0. apply beq_nat_false in H0. omega.

  reflexivity.

  (* subgoal:  asnat (st X) + 1 + asnat (update st Y (VNat (asnat (st X) + 1)) X) = x *)
  unfold update. remember (beq_id Y X) as Q. 
  destruct Q.
    simpl. inversion HeqQ.
    inversion HeqQ. rewrite <- H. inversion HeqQ.
  admit.   (* ??? *)

  (* subgoal:  asnat (st Y) = x *)
  apply beq_nat_true_iff in H0. rewrite -> H0 in  H. simpl in H. assumption.
Admitted.

(*

(* from https://github.com/suharahiromichi/coq/blob/master/sf/coq_sf_hoare_formal_pilkiewicz.v *)
Example slow_assignment_dec (x:nat) : dcom := (
  ...
  END    (* this needs to revise the definition of dcom. *) 
) % dcom.
...
Example fact_com_dec (x : nat) : dcom := (
    {{ fun st => asnat (st X) = x }} =>
    {{ fun st => real_fact (asnat (st X))  = real_fact x  }}
  Z ::= AId X
    {{ fun st => real_fact (asnat (st Z)) = real_fact x }} =>
    {{ fun st => 1 * real_fact (asnat (st Z))  = real_fact x  }} ;
  Y ::= ANum 1
...
Proof.
  intros. verify.
  (* asnat (st Z) <> 0 *)
  rewrite -> negb_true_iff in H0. apply beq_nat_false in H0. assumption.

  (* negb (beq_nat (asnat (st Z)) 0) = true *)
  rewrite -> negb_true_iff. apply beq_nat_false_iff. assumption.

  (* asnat (st Z) = 0 *)
  apply eq_true_negb_classical in H0.  apply beq_nat_true in H0. assumption.

  (* negb (beq_nat (asnat (st Z)) 0) <> true *)
  unfold negb. destruct (st Z).
    rewrite -> H0. simpl. apply diff_false_true.
    rewrite -> H0. simpl. apply diff_false_true.

  (* asnat (st Y) * asnat (st Z) * real_fact (asnat (st Z) - 1) = real_fact x *)
  induction (asnat (st Z)).
  Case "Z = 0".
    inversion H0.
  Case "Z = S n".
    simpl. rewrite <- minus_n_O. cut (S n * real_fact n = real_fact (S n)). intros H1. 
    rewrite <- mult_assoc. rewrite -> H1. assumption.
    (* prove cut *)
    induction n.
      reflexivity.
      simpl; reflexivity.

  (* asnat (st Y) = real_fact x *)
