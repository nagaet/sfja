...
Proof.
  ...
  Case "Loop body preserves invariant".
   ...
   (* kokomade setsumei shita... (2015/4/4) *)
...
Proof. 
  ...
  eapply hoare_consequence_post.            (* (5) *)
...
Fixpoint snoc {X:Type} (l:list X) (v:X) : (list X) :=      (* snoc l v で、v をl の末尾につける。*)
  match l with
  | nil      =>  [ v ]
  | cons h t => h :: (snoc t v)
  end.
...
Proof.
  ...
  unfold not in H0. rewrite negb_true_iff in H0. rewrite -> beq_nat_false_iff in H0. admit.

  unfold not. intros H1. rewrite -> beq_nat_true_iff in H0. rewrite -> H0 in H1. simpl in H1.  inversion H1.

  rewrite -> update_neq. rewrite -> plus_comm. simpl. rewrite -> plus_n_Sm. admit.
  reflexivity.

  assert (H1: update st Y (VNat (asnat (st X) + 1)) X = st X). apply update_neq. reflexivity.
