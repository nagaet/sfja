...
Proof.
  intros P Q H.
  inversion H as [HP HQ].  (* 仮定にはinversion *)
  apply HP.  Qed.
...
Proof.
  ...
  Case "n = 0". split.
    ...
    SCase "right". intros H. inversion H.
  Case "n = S n' ". split.
    SCase "left". intros H. apply proj2 in IHn'. apply IHn'. exact H.
    SCase "right". intros H. apply ev_SS. apply proj1 in IHn'. apply IHn' in H. exact H. Qed.
  (* ヒント: nに帰納法を使います. *)
...
P /\ Q -> Q /\ R -> P /\ R.
Proof.
  intros P Q R H H0. apply proj1 in H. apply proj2 in H0.
  split.
  (* Case "left". *) exact H.
  (* Case "right". *) exact H0. Qed.
Print conj_fact'.

(* for cheat 2 *)
Definition conj_fact'' : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
fun (P Q R : Prop) (H : P /\ Q) (H0 : Q /\ R) =>
let H1 := proj1 P Q H in
let H2 := proj2 Q R H0 in
conj P R H1 H2.

Definition conj_fact : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
fun (P Q R : Prop) (H : P /\ Q) (H0 : Q /\ R) =>
conj P R (proj1 P Q H) (proj2 Q R H0). 
...
Proof.
  intro P. split.
  Case "->". intros H. exact H.
  Case "<-". intros H. exact H. Qed.

Theorem iff_trans : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R H H0. split.
  Case "->".
    inversion H as [HAB HBA]. inversion H0 as [H0AB H0BA].
    intros H1. apply HAB in H1. apply H0AB in H1. exact H1.
  Case "<-".
    inversion H as [HAB HBA]. inversion H0 as [H0AB H0BA].
    intros H1. apply H0BA in H1. apply HBA in H1. exact H1. Qed.
...
Proof.
  ...
  intros. apply H. apply H0. apply H1. Qed.
...
(** ヒント: もしコンテキストに iff を含む仮定があれば、 
    ...
    (なぜそうできるのか考えてみましょう。)  
    2つの含意(->)をandでつないだものだから。コンテキストでinversionが使える。*)
(** [] *)
...
(* for cheat *)
Theorem MyProp_iff_ev' : forall n : nat,
  MyProp n <-> ev n.
Proof.
  intros n. split.
  apply ev_MyProp.
...
let H0 :=
  match H with
  | or_introl HP => let Case := "right" in or_intror Q P HP
  | or_intror HQ => let Case := "left" in or_introl Q P HQ
  end in
H0
     : forall P Q : Prop, P \/ Q -> Q \/ P
...
Theorem or_commut'' : forall P Q R : Prop,
  P \/ Q \/ R  -> R \/ Q \/ P .
Proof.
  intros P Q R H.
  inversion H as [HP | [ HQ | HR] ].
    Case "right". right. right. apply HP.
    Case "middle". right. left. apply HQ.
    Case "left". left. apply HR.  Qed.
...
Theorem or_distributes_over_and : forall P Q R : Prop,
  P \/ (Q /\ R) <-> (P \/ Q) /\ (P \/ R).
Proof.
  intros P Q R. apply iff_sym. split.
  Case "->".
    intros H. apply or_distributes_over_and_2 in H. apply H.
  Case "<-".
    intros H. apply or_distributes_over_and_1 in H. apply H. Qed.
...
Proof.
  ...
  Case "b = false". destruct c.
    ...
    SCase "c = false". left. reflexivity. Qed. 
...
Proof.
  ...
  Case "b = false". destruct c.
    ...
    SCase "c = false". simpl in H. inversion H. Qed.
...
Proof.
  intros b c H. destruct b.
  Case "b = true". destruct c.
    SCase "c = true". unfold orb in H. inversion H.
    SCase "c = false". unfold orb in H. inversion H.
  Case "b = false". destruct c.
    SCase "c = true". unfold orb in H. inversion H.
    SCase "c = false". unfold orb in H. split. exact H. exact H. Qed. 
...
(** Coqで否定を扱えるようになるにはある程度慣れが必要です。
    ...
    最初のうちはなかなか大変です。ウォームアップのつもりで、否定に関する
    馴染みのある定理を取り上げてみましょう。
 *)
...
Theorem contrapositive : forall P Q : Prop,
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H H0. unfold not. unfold not in H0. intros H1. apply H in H1. apply H0 in H1. inversion H1.
  Qed.
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros H. inversion H as [H0 H1].
...
Proof.
  ...
  Case "S (S (S n))".
    intros H0. inversion H0 as [|n0 H2 H1]. apply IHev in H2. inversion H2. Qed.

(*  **** Exercise: 1 star (informal_not_PNP) *)
(** **** 練習問題: ★ (informal_not_PNP) *)
(*  Write an informal proof (in English) of the proposition [forall P
    : Prop, ~(P /\ ~P)]. *)
(** 命題 [forall P : Prop, ~(P /\ ~P)] の形式的でない証明を書きなさい。 
...
(* FILL IN HERE *)
(** 排中律と2重否定除去法則の同値は「記号論理入門」 p.57, 58 ?? 
    5番めの式と排中律についてはp.115
    4番目の式はド・モルガンの法則と二重否定除去の法則。
 *)
...
Theorem not_eq_beq_false : forall n n' : nat, 
     ...
     beq_nat n n' = false.
Proof.
  intros n n' H. unfold not in H.  generalize dependent n'.
  induction n as [| n''].
  Case "n =0". 
    destruct n' as [| n'''].
    SCase "n' = 0".
      simpl. intros H. apply ex_falso_quodlibet. apply H. reflexivity.
    SCase "n' = S n''' ".
  ...
  Case "n = S n'' ". 
    ...
    SCase "n' = 0".
      simpl. intros H. reflexivity. 
    SCase "n' = S n''' ".
      simpl. intros H. apply IHn''. intros H0. apply H. rewrite H0. reflexivity. Qed.
...
Proof.
  ...
  Case "n = 0".
    ...
    SCase "m = S m". 
      simpl in H. intros H0. inversion H0. *)
  Case "n = S n".
    intros H0. rewrite -> H0 in H. rewrite <- beq_nat_refl in H. inversion H. Qed.
...
(** Coqの容易な表記法の定義は、存在量化された命題を記述するための、より
    馴染みやすい表記を、ビルトインされた全称量化子と同レベルで実現しています。
...
(** 逆に、コンテキストに置かれた仮定の中に存在を示すものがある場合は、
   ...
   ==> 「名前を明示的に選ばないと、Coqはそれを[witness]としてしまいます。
             これは、証明を混乱させるものにしてしまいます。」　のほうがいいと思います。
 *)
 ...
 *)
Example exists_add1_ev : exists n,
     ev (S n).
Proof.
  exists 3. apply ev_SS. apply ev_SS. apply ev_0. Qed.
Print exists_add1_ev.
(* exists_add1_ev = 
    ex_intro nat (fun n : nat => ev (S n)) 3 (ev_SS 2 (ev_SS 0 ev_0))
       : exists n : nat, ev (S n)      *)
...
Proof.
  intros X P H. unfold not. intros H0. inversion H0 as [x H1]. apply H1. apply H. Qed.
...
Proof.
  unfold excluded_middle. intros excl_mid. intros X P H x.
  assert ( P x \/ ~ P x).
  Case "Proof of assertion".
    apply excl_mid.
  inversion H0. apply H1. apply ex_falso_quodlibet. unfold not in H. apply H. unfold not in H1.
  exists x. apply H1. Qed.
...
Proof.
  intros X. intros P Q. unfold iff. split.
  Case "->".
  ...
  Case "<-".
    intros H. inversion H.
    inversion H0 as [x H1]. exists x. left. apply H1.
...
Proof.
  ...
  Case "<-".
    intros H. inversion H. apply refl_equal. Qed.
...
(** これまでにも [inversion] が等値性にからむ仮定や帰納的に定義された命題に対して
    ...
    - その型 [P] の定義にある各コンストラクタ [C] が、
      ...
      - [C] の結論（戻り値の型）を現在のゴールとmatchして、 [C] を適用できるような一連の等式を算出する。
...
(** この定義はかなり直観的なものになります。これは、ある数値がもう一つの
    ...
    示しています。一つはそれらが同じ数であるかどうかを確認すること。
    もう一つは、最初の数が二つ目の数の一つ前の数より小さいかまたは等しい、
    ということの根拠を得ることです。
 *)
...
(** これはこれで [<=] という関係の妥当な定義だと言えます。しかし少し観察してみると
    定義の左側に現れる [n] は全て同じだということがわかります。ということは、
    個々のコンストラクタにではなく定義全体に全称量化子 (つまり "->") を使うことが
...
Example test_next_even :
  next_even 2 4.
Proof.
  apply ne_2.  apply ev_SS. apply ev_SS. apply ev_0. Qed.
...
(** 二つの自然数のペア同士の間に成り立つ帰納的な関係 [total_relation] を
    定義しなさい。 *)
(*  ==>「2つの自然数のペア」という言い方?
            ここは英語の文のように、「一対の自然数の間で成り立つ帰納的な関係」と書いたほうがよいのでは？
            私はここの説明を読んで, 自然数のペアを2つという前提で考えてしまいました。 *)

(* 
関係演算子 ≦ には、どんな自然数 a, bを選んでも、
a ≦ b
もしくは
b ≦ a
の、少なくともどちらか一方が必ず成り立ちます。
このような順序関係のことを全順序関係(total order)といいます。
参考： http://www.geocities.jp/k27c8_math/math/relation/totally_orderd_relation_and_partial_orderd_relation.htm
*)

Inductive total_relation (Rel : nat -> nat -> Prop) : Prop :=
   total :  forall a b, Rel a b \/ Rel b a  -> total_relation Rel.
...
Inductive empty_relation : nat -> nat -> Prop :=
  | empty_relation_0 : forall a b, empty_relation a b.
Check empty_relation_ind.

Example test_empty_relation_1 :
  empty_relation 3 4.
Proof.
  apply empty_relation_0. Qed.
...
(**  - 次の命題のうち、この関係を満たすと証明できると言えるのはどれでしょうか。
      - [R 1 1 2]
 ==> 証明できる。
  ([R 1 1 2]) に apply c2, apply c3 すると c1 : [R 0 0 0] になる。

      - [R 2 2 6]
==> 証明できない。
 [R 2 2 6] に apply c3, apply c3を適用すると、[R 2 0 4]
さらにapply c2, apply c2すると、[R 0 0 2]。[R 0 0 0]には移れない。


    - この関係 [R] の定義からコンストラクタ [c5] を取り除くと、
      証明可能な命題の範囲はどのように変わるでしょうか？端的に（１文で）説明しなさい。
==> 変わらない. c5を外すと[R n m o]を[R m n o]に変えられなくなるが、
      m + n = o のとき証明できるというのは変わらないので。

    - この関係 [R] の定義からコンストラクタ [c4] を取り除くと、
       証明可能な命題の範囲はどのように変わるでしょうか？端的に（１文で）説明しなさい。
==> 変わらない. 
       c2, c3はapplyでRの数字を減らす方向(例 R 3 2 3 を R 2 2 2にする)で、
       c4はRの数字を増やす方向(例 R 1 1 1 を R 2 2 3にする)。
       Rの数字は自然数だから、c4がなくても証明できる範囲は変わらないはず。
**)
...
(** 関係 [R] の、等値性に関する特性をあげ、それを証明しなさい。 それは、
  ...
  ===> 「m, n, oについてどんなことが言えるでしょうか？」と訳すべきだと思います。
　　　もとの訳だと、m, n, oについて独立な関係をつくるように読めます。
 *)
...
Proof.
  ...
  Case "subgoal4".
    ...
    apply eq_add_S. apply eq_add_S. apply IHR.
  Case "subgoal5".
    rewrite <- IHR. rewrite plus_comm. reflexivity. Qed.
...
Proof.
  ...
  Case "R 0 n (0 + n)".
    ...
    SCase "R 0 0 (0 + 0)".
      apply c1.
    SCase "R 0 (S n) (0 + S n)".
      apply c3. apply R_0_n_n.
  Case "R (S m) n (S m + n)".
    destruct n as [| n'].
    SCase "R (S m) 0 (S m + 0)".
      simpl. apply c2. rewrite -> plus_0_r. apply R_n_0_n.
    SCase "R (S m) (S n) (S m + S n)".
      simpl. apply c2. rewrite <- plus_n_Sm. apply c3. apply IHm'.
Qed.

Theorem R_fact2 :    
  forall m n o, m + n = o -> R m n o.
Proof.
  intros m n o H. induction m as [| m'].
  Case "R 0 n o".
    destruct n as [| n'].
    SCase "R 0 0 o".
      simpl in H. rewrite <- H. apply c1.
    SCase "R 0 (S n) o".
      simpl in H. rewrite <- H. apply c3. apply R_0_n_n.
  Case "R (S m) n o".
    destruct n as [| n'].
    SCase "R (S m) 0 o".
      simpl in H. rewrite -> plus_0_r in H. rewrite -> H. rewrite -> plus_0_r in IHm'. apply R_n_0_n.
    SCase "R (S m) (S n) o".
      rewrite <- H. simpl. rewrite <- plus_n_Sm. apply c2. apply c3. apply R_m_n_mplusn.
Qed.
...
Proof.
  ...
  Case "->".
    ...
    SCase "nil".
      apply all_nil.
    SCase "x :: l' ".
      simpl in H. apply andb_true__and in H. inversion H. apply IHl' in H1. apply all_cons. apply H1. apply H0. 
  Case "<-".
    intros H. induction H as [|l' x H1 H2]. 
    SCase "nil".
      simpl. reflexivity.
    SCase "x :: l' ". 
      simpl. apply and__andb_true. split. apply H. apply H2. Qed.
...
(* first try 
Inductive filter' (X:Type) (test:X->bool) : list X -> list X :=
  | filter'_nil : filter' X test nil
  | filter'_l1_cons : forall (l:list X) (x : X), filter' X test l -> test x -> filter' X test (x :: l)
  | filter'_l2_cons : forall (l:list X) (x : X), filter' X test l -> ~ (test x) -> filter' X test l.

Theorem filter_challenge_firsttry : forall (X : Type) (test : X -> bool) (l1 l2 l : list X),
  filter test l = l1 -> in_order_merge X l1 l2 l.
Proof.
  intros X test l1 l2 l H. induction l1. destruct l2.
  Admitted.
*)
...
(* 初期状態(nil)から帰納的に構成できるように書く。*)
Inductive in_order_merge (X:Type) : list X -> list X -> list X -> Prop :=
  | in_order_merge_nil : in_order_merge X nil nil nil
...
in_order_merge [] [3] [3]
-------------------------- in_order_merge_left
in_order_merge [2] [3] [2,3]
------------------------------  in_order_merge_left
in_order_merge [6,2] [3] [6,2,3]
-------------------------------  in_order_merge_right
in_order_merge [6,2] [4,3] [4,6,2,3]
--------------------------------      in_order_merge_left
...
Lemma eq_remove_cons : forall (X:Type) (l1 l2: list X) (x: X),
  l1 = l2 -> x :: l1 = x :: l2.
Proof.
  intros X l1 l2 x H. induction l1 as [| n l1'].
  Case "l1 = nil".
    destruct l2 as [| m l2'].
    SCase "l2 = nil".
      reflexivity.
...
Theorem filter_challenge' : forall (X : Type) (test : X -> bool) (l1 l2 l3 : list X),
  in_order_merge X l1 l2 l3 ->
  all X (fun x => test x = true) l1 ->
  all X (fun x => test x = false) l2 ->
  filter test l3 = l1.
Proof.
  intros X test l1 l2 l3 H H0 H1.  induction H as [| x l1' l2 l3' | x' l1 l2' l3']. 
  Case "merge_nil".
    simpl.  reflexivity.
  Case "merge_left".
    inversion H0. simpl. rewrite -> H5. apply eq_remove_cons. apply IHin_order_merge. apply H4. apply H1.
  Case "merge_right".
    inversion H1. simpl. rewrite -> H5. apply IHin_order_merge. apply H0. apply H4. Qed. 

(*
Lemma eq_remove_cons : forall (X:Type) (l1 l2: list X) (x: X),
  l1 = l2 -> x :: l1 = x :: l2.
Proof.
  intros X l1 l2 x H. induction l1.
  Case "l1 = nil". destruct l2.
    SCase "l2 = nil". reflexivity.
    SCase "l2 = x :: l2' ". rewrite <- H. reflexivity.
  Case "l1 = x :: l1' ". destruct l2.
    SCase "l2 = nil". rewrite -> H. reflexivity.
    SCase "l2 = x :: l2' ". rewrite <- H. reflexivity. Qed.
...
Theorem filter_challenge : forall (X : Type) (test : X -> bool) (l1 l2 l3 : list X),
  ...
  filter test l3 = l1.
Proof.
  intros X test l1 l2 l3 H H0 H1.
  induction H as [| x l1' l2 l3' | x' l1 l2' l3'].
  Case "merge_nil".
    reflexivity.
  Case "merge_left".
    inversion H0. simpl. rewrite -> H5. apply eq_remove_cons. apply IHin_order_merge. apply H4. apply H1.
...
Inductive sublist (X : Type) : list X -> list X -> Prop :=
  | sublist_nil      : forall l : list X, sublist X nil l
  | sublist_cons1 : forall (x : X) (l1 l2 : list X),
    sublist X l1 l2 -> sublist X l1 (x :: l2)
  | sublist_cons2 : forall (x : X) (l1 l2 : list X),
    sublist X l1 l2 -> sublist X (x :: l1) (x :: l2).

Lemma length_filter_test_list : forall (X : Type) (test : X -> bool) (l : list X),
  0 <= length (filter test l).
Proof.
  intros X test l. induction l.
    simpl. apply le_n.
    simpl. destruct (test x). simpl. apply le_S. apply IHl.
    apply IHl.
Qed.

Theorem filter_challenge_2_firsttry : forall (X : Type) (test : X -> bool) (l1 l2 : list X),
  sublist X l1 l2 ->
  length l1 <= length (filter test l2).
Proof.
  intros X test l1 l2 H. generalize dependent l2. induction l1.
  Case "l1 = nil".
    destruct l2.
      intros H. simpl. apply le_n.
      intros H. apply length_filter_test_list.
  Case "l1 = x :: l1' ".
    destruct l2.
      intros H. inversion H.
      intros H.   (* 証明できない。。。 *)
...
(* https://github.com/timjb/software-foundations/blob/master/Logic.v を参考にしました。 *)
...
Theorem filter_subseq : forall (X: Type) (test : X -> bool) (l : list X),
  subseq X (filter test l) l.
Proof.
  intros X test l. induction l as [| x xs].
  Case "nil".
    simpl. apply nil_subseq.
  Case "x :: xs".
...
Proof.
  intros n m H. induction H as [| H'].
    apply le_n.
    apply le_S. apply IHle.
Qed.

Theorem filter_challenge_2 : forall X (test : X -> bool) (s l : list X), 
  subseq X s l -> forallb test s = true -> length s <= length (filter test l).
Proof.
  intros X test s l. generalize dependent s. induction l as [|x l'].
  Case "l = nil".
    intros s H H0. inversion H. simpl. apply le_n.
  Case "l = x l' ".
    intros s H H0. remember (test x) as test_x. destruct test_x.
    SCase "test_x = true".
      inversion H.
      SSCase "nil".
        simpl. apply le_0_n.
      SSCase "s".
        simpl. rewrite <- Heqtest_x. simpl. apply le_S. apply IHl'.
          apply H3.
          apply H0.
       SSCase "x :: l1".
        simpl. rewrite <- Heqtest_x. simpl. apply le_n_m_Sn_Sm. apply IHl'.
          apply H3.
          rewrite <- H2 in H0. simpl in H0. rewrite -> H1 in H0. rewrite <- Heqtest_x in H0. simpl in H0. apply H0.
    SCase "test_x = falsehood".
      inversion H.
      SSCase "nil".
        simpl. apply le_0_n.
      SSCase "s".
        simpl. rewrite <- Heqtest_x. apply IHl'.
          apply H3.
          apply H0.
      SSCase " x :: l1".
        rewrite <- H2 in H0. simpl in H0. rewrite -> H1 in H0. rewrite <- Heqtest_x in H0. inversion H0.
Qed.


(* Do I have to use http://ccvanishing.hateblo.jp/entry/2012/12/30/205251 ??  *)
...
Proof.
  intros X xs ys x H. inversion H. 
  induction H0.
    apply ai_here.
    simpl. apply ai_later. apply IHappears_in. left. apply H0.
  induction xs.
    apply H0.
    simpl. apply ai_later. apply IHxs. right. apply H0. Qed. 
...
(* first try 
Inductive disjoint_firsttry (X: Type): list X -> list X -> Prop :=
  | disjoint_nil1 : forall l : list X, disjoint_firsttry X nil l
  | disjoint_nil2 : forall l : list X, disjoint_firsttry X l nil
  | disjoint_cons1 : forall (a:X)(l1 l2: list X), 
    disjoint_firsttry X l1 l2 -> ~(appears_in a l2) -> disjoint_firsttry X (a :: l1) l2
  | disjoint_cons2 : forall (a:X)(l1 l2: list X), 
    disjoint_firsttry X l1 l2 -> ~(appears_in a l1) -> disjoint_firsttry X l1 (a :: l2).
*)
...
Inductive no_repeats {X:Type} : list X -> Prop :=
  | no_repeats_nil: no_repeats nil
  | no_repeats_cons: forall (a:X) (l: list X), no_repeats l -> ~(appears_in a l) -> no_repeats (a :: l).
...
Lemma disjoint_cons : forall (X : Type) (l1 l2 : list X) (x : X),
  disjoint (x :: l1) l2 -> disjoint l1 l2.
Proof.
  intros X l1 l2 x H. unfold disjoint.
  intros x' contra. destruct contra as [ail1 ail2].

  unfold disjoint in H. unfold not in H. apply H with (x0 := x'). split.
  apply ai_later. apply ail1.
  apply ail2. Qed.

Lemma no_repeats_rest : forall (X : Type) (l1 l2 : list X) (x : X),
no_repeats (x :: l1) -> no_repeats l1.
Proof.
  intros X l1 l2 x H. inversion H. apply H2. Qed.

Theorem norepeats_disjoint : forall (X : Type) (l1 l2 : list X),
  disjoint l1 l2 -> no_repeats l1 -> no_repeats l2 -> no_repeats (l1 ++ l2).
Proof.
  intros X l1 l2 H H0 H1. induction l1 as [| x l1'].
  Case "l1' = nil".
    simpl. apply H1.
  Case "l1 = n l1' ".
    simpl. apply no_repeats_cons. apply IHl1'. apply disjoint_cons in H. apply H.
    apply no_repeats_rest in H0. apply H0. apply l2.
    unfold not. intros H2. apply appears_in_app in H2.
    unfold disjoint in H. unfold not in H. apply H with (x0 := x).
    split. apply ai_here. 
    inversion H0. unfold not in H6.  
    inversion H2 as [H2P | H2P]. apply H6 in H2P. inversion H2P. apply H2P. Qed.



 (* 以下の定理は https://github.com/timjb/software-foundations/blob/master/Logic.v からのものです。*)

Theorem no_repeats_2 : forall (X : Type) (x : X) (l1 l2 : list X),
  appears_in x l1 -> appears_in x l2 -> ~ (no_repeats (l1 ++ l2)).
Proof.
  intros X x l1 l2 ail1. induction ail1 as [| ail1'].
    intros ail2 contra. inversion contra. apply H2. apply app_appears_in. right. apply ail2.
    intros ail2 contra. inversion contra. apply IHail1. apply ail2. apply H1.
Qed.

Theorem no_repeats_3 : forall {X : Type} (l1 l2 : list X), no_repeats (l1 ++ l2) -> disjoint l1 l2.
Proof.
  intros X l1 l2 nr. unfold disjoint. intros x' contra. destruct contra as [ail1 ail2]. 
  apply not_both_true_and_false with (P := no_repeats (l1 ++ l2)). apply conj.
    apply nr.
...
Proof.
  ...
  Case "m = S m' ". 
    inversion H. apply le_n. apply le_S. apply IHm'. apply H1. Qed.
...
Proof.
  ...
  Case "m = 0".
    intros H. inversion H. apply le_n. inversion H1.
  Case "m = S m' ".
...
(*
Lemma le_plus_r: forall a b c,
  a <= b -> a <= b + c.
Proof.
  intros a b c H. induction a as [| a'].
  apply O_le_n. apply IHa' in H. apply le_S in H. apply Sn_le_Sm__n_le_m in H. apply IHa' in H.
  apply n_le_m__Sn_le_Sm in H.
  apply n_le_m__Sn_le_Sm.

Lemma le_plus_r2 : forall a b c,
  a + b <= c -> a <= c.
Proof.
  intros a b c H. generalize dependent b. induction a as [| a'].
  intros. apply O_le_n. 
  intros. destruct b.
  simpl in H. simpl in H. rewrite -> plus_0_r in H. apply H. 
  apply le_S in H. simpl in H. rewrite <- plus_n_Sm in H. apply le_S in H. 
  apply Sn_le_Sm__n_le_m in H. apply Sn_le_Sm__n_le_m in H.  apply IHa' in H. 

  apply Sn_le_Sm__n_le_m in H. apply IHa' in H. 
Admitted.
*)
...
Theorem plus_lt : forall n1 n2 m,
  n1 + n2 < m ->
  n1 < m /\ n2 < m.
Proof.  (* https://github.com/timjb/software-foundations/blob/master/Logic.v を参考にしました *)
  intros n1 n2 m. generalize dependent n1. generalize dependent n2. induction m as [| m'].
  Case "m = 0".
    intros n1 n2 H. inversion H.
    intros n1 n2 H. destruct n1 as [| n1'].
    SCase "n1 = 0".
      destruct n2 as [| n2'].
        SSCase "n2 = 0".
          split.
        ...
        SSCase "n2 = S n2' ".
          split.
            ...
            apply n_le_m__Sn_le_Sm. apply O_le_n.
  Case "m = S m' ".
    rewrite <- plus_n_Sm in H. apply Sn_le_Sm__n_le_m in H. apply IHm' in H.  inversion H as [HP HQ]. 
      split.
        apply le_S. apply HP.
        apply n_le_m__Sn_le_Sm. apply HQ. Qed.
    ...
    destruct n2 as [| n2'].
      ...
      SCase "n2 = S n2' ".
        split.
          ...
          unfold lt. apply n_le_m__Sn_le_Sm. simpl.  apply le_n.
  Case "n1 = S n1' ".
    inversion H0. split.
      ...
      apply le_S in H0. apply Sn_le_Sm__n_le_m in H0. apply le_plus_r2 in H0. apply H0.
      apply le_S in H0. apply Sn_le_Sm__n_le_m in H0. rewrite -> plus_comm in H0. apply le_plus_r in H0. 
      unfold lt. apply n_le_m__Sn_le_Sm. apply H0.
*)
...
Theorem lt_S : forall n m,
  ...
  n < S m.
Proof.
  intros n m H. unfold lt. generalize dependent m. induction n as [| n'].
  Case "n = 0".
    intros m H. destruct m as [| m'].
    SCase "m = 0".
      unfold lt in H. apply le_n.
    SCase "m = S m' ".
      unfold lt in H. apply le_S. apply H.
  Case "n = S n' ".
    intros m H. unfold lt in H. apply le_S. apply H. Qed.
...
Proof.
  ...
  Case "n = S n' ".
    ...
    SCase "m = 0".
      simpl in H. inversion H.
    SCase "m = S m' ".
      simpl in H. apply n_le_m__Sn_le_Sm. apply IHn'. apply H. Qed.



(* Lemma for ble_nat_n_Sn_false *)
Lemma ble_nat_n_m_false: forall n m,
  ble_nat n m = false ->
  ble_nat (S n) (S m) = false.
Proof.
  intros n m H. generalize dependent m. induction n as [| n'].
  Case "n  = 0".
...
(* Lemma for ble_nat_n_Sn_false *)
Lemma ble_nat_Sn_Sm_false: forall n m,
  ble_nat (S n) (S m) = false ->
  ble_nat n m = false.
Proof.
  intros n m H. generalize dependent m. induction n as [| n' ].
...
Proof.
  intros n m H. generalize dependent m. induction n as [| n' ].
  Case "n = 0".
    intros m H. inversion H. (* destruct m as [| m' ].
    SCase "m = 0".
      inversion H.
    SCase "m = S m' ".
      unfold ble_nat in H. inversion H. *)
  Case "n = S n' ".
  intros m H. simpl in H. destruct m as [| m' ].
    SCase "m = 0".
      unfold ble_nat. reflexivity.
    SCase "m = S m' ".
      apply ble_nat_n_m_false. apply IHn'. apply H. Qed.


(* wrong 
Lemma ble_nat_Sn_n_false : forall n m,
  ble_nat (S n) m = false ->
  ble_nat n m = false.
Proof.
Admitted.*)
...
Proof.
  unfold not. intros n m H H0. generalize dependent n. induction m as [| m' ].
  Case "m = 0".
    intros n H H0. inversion H0. rewrite -> H1 in H. inversion H.
  Case "m = S m' ".
    intros n H H0. destruct n as [| n' ].
    SCase "n = 0".
      inversion H.
    SCase "n = S n' ".
      apply IHm' in H. inversion H. apply Sn_le_Sm__n_le_m in H0. apply H0. Qed.
...
Inductive nostutter:  list nat -> Prop :=
  ...
  | nostutter_cons : forall (n m : nat) (l : list nat), nostutter (m :: l) -> beq_nat n m = false -> nostutter (n :: m :: l).
...
Proof.
  apply nostutter_cons. apply nostutter_cons. apply nostutter_cons. apply nostutter_cons. apply nostutter_cons.
  apply nostutter_one.
  simpl. reflexivity. reflexivity. reflexivity. reflexivity. reflexivity. Qed.
...
(** 「鳩の巣定理（ "pigeonhole principle" ）」は、「数えあげる」という
...
Proof.
  intros X x l H. induction H as [| l'].
  Case "x :: l".
    exists []. exists l. reflexivity.
  Case "l' :: l".
    inversion IHappears_in as [l'' H0]. exists (l' :: l''). inversion H0 as [l''' H1].
    exists l'''. rewrite -> H1. reflexivity. Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  | repeats_start: forall (a : X), repeats  (a :: [ a ])
  | repeats_insert: forall (a: X) ( l1 l2 : list X), repeats (l1 ++ l2) -> repeats (l1 ++ a :: l2). 
*)
...
Theorem pigeonhole_principle': forall {X:Type} (l1 l2:list X),
  ...
  repeats l1.
Proof.
  intros X l1. induction l1 as [| n' l1'].
  intros l2 excl_mid H0 H1.
  inversion H1. intros l2 H H0 H1.
...
Proof.
  ...
  intros l2 excl_mid H0 H1. inversion H1.
  intros l2 excl_mid H0 H1.
  remember (excl_mid (appears_in x l1)) as excl_mid_appears_in.
  inversion excl_mid_appears_in as [appears | appears_not].
  apply repeats_here. apply appears.
  apply repeats_later. 

  destruct (appears_in_app_split x l2 (H0 x (ai_here x l1))) as [w1 [w2]].
    (* appears_in_app_split forall: {X:Type} (x:X) (l:list X),appears_in x l -> exists l1, exists l2, l = l1 ++ (x::l2).
          を使って
        H : l2 = w1 ++ x :: w2
          となるように分ける。それには、以下のものも使う。
        H0 : forall x0 : X, appears_in x0 (x :: l1) -> appears_in x0 l2
        ai_here : forall l, appears_in a (a::l)      *)

    apply IHl1 with (l2 := w1 ++ w2).
    SCase "excluded_middle".
      apply excl_mid.
    SCase "appears_in x l1 -> appears_in x l2".
      intros x0 H2. assert (H2' := H2). apply ai_later with (b := x) in H2. apply H0 in H2. rewrite -> H in H2.
      apply appears_in_app in H2. inversion H2 as [inw1 | inxw2].
        apply app_appears_in. left. apply inw1.
        apply app_appears_in. right. inversion inxw2.
          rewrite -> H4 in H2'. apply contradiction_implies_anything with (P := appears_in x l1).
                (* contradiction_implies_anything: forall P Q : Prop, P /\ ~P -> Q *)
          split. apply H2'. apply appears_not. apply H4.
    SCase "length l2 < length l1".
      rewrite -> H in H1. rewrite -> app_length in H1. simpl in H1. rewrite <- plus_n_Sm in H1.
      apply Sn_le_Sm__n_le_m in H1. rewrite <- app_length in H1. apply H1. Qed.
...
(** これは次のように読めます :
     ...
     [F] は [n] について以下のようなパターンマッチをしています：
      ...
      - もし [S n0] ならば、[F] は [P n0] が成り立つ根拠を手に入れるために、
         [n0] を持ってそれ自身を再帰呼び出しする。
        そうして得た根拠が [f0] に適用され [P (S n)] が成り立つことが示される。
...
(** ここで一つの疑問が起こってきます。自動化された証明支援系が
    ...
    - Coq の型はそれ自身が式となっているため、その型チェッカがそれらを比較する前際に、
      変換ルールに基づいて正規化しなければならない。

    - 型チェッカは、 [match] の式が「尽くされている（_exhaustive_ ）ことを確認しなければならない。
      つまり、その型にあるコンストラクタに対応する枝をすべて持っていなければならい。
      その理由は、次に提示された証明オブジェクトについて考えればわかるはずです。
...
[[
    ...
    - 型チェッカは、各 [fix] の式が終了することを確認しなければならない。
      これは文法レベルで「各々の再帰呼び出しに元々の引数にわたってきた式の部分式が渡されていること」を
      チェックをすることで実現されている。
      この理由の本質的なところを理解するために次の証明について考えてください。
[[
          Definition nat_false : forall (n:nat), False :=
             fix f (n:nat) : False := f n.
]]
